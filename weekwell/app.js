...
(function () {
  ...
  function viewGroup() {
    ...
    return `
      <div class="card"><div class="row"><div>${ring(groupPct(), t('group_pct'))}</div><div class="grow"><div class="row" style="font-weight:800">${gav(30)} <span>${esc(S.group.name)}</span> ${S.group.admin === S.me ? `<button class="mini ghost" data-gsettings="1">✏️</button>` : ''}</div><div class="row mt" style="align-items:flex-end;height:60px">${ghist || `<span class="muted small">${t('no_items')}</span>`}</div></div></div></div>
