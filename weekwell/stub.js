...
(function () {
  ...
  const ST = {
    ...
    async sendMessage(text, replyTo) { S.messages.push({ id: uid(), user: S.me, text, ts: Date.now(), replyTo: replyTo || null, reactions: {} }); save(); },
    async toggleMsgReaction(mid, emoji) { const m = S.messages.find((x) => x.id === mid); if (!m) return; m.reactions = m.reactions || {}; const a = m.reactions[emoji] = m.reactions[emoji] || []; const i = a.indexOf(S.me); if (i >= 0) a.splice(i, 1); else a.push(S.me); if (!a.length) delete m.reactions[emoji]; save(); },
    async deleteMessage(mid) { S.messages = S.messages.filter((x) => x.id !== mid); save(); },
    ...
    async addActivity(p) { let url = null; if (p.blob) { url = await blobUrl(p.blob); S.photos.unshift({ id: uid(), user: S.me, url, caption: p.title, kind: 'activity', ts: Date.now(), expires: null }); } S.events.unshift({ id: uid(), user: S.me, type: 'activity', a: { atype: p.atype, value: p.value, unit: p.unit, note: p.note || '', title: p.title, date: p.date, cc: p.cc || null }, photo: url, ts: Date.now(), kudos: {}, comments: [] }); if (p.cc) { const k = S.me + '|' + p.cc + '|' + p.date; const cur = S.logs[k] || {}; S.logs[k] = { ...cur, value: (cur.value || 0) + p.value, done: true }; } save(); },
    async deleteActivity(id) { S.events = S.events.filter((e) => e.id !== id); save(); },
