...
(function () {
  ...
  function viewHome() {
    ...
    return `
      ...
      ${(S.next && S.next.simulated && S.next.simulated.length && !S.cur.challenges.length) ? `<div class="card"><h3>${t('next_week')} · ${S.next.week_start || ''}</h3>${S.next.simulated.map((o) => `<div class="row" style="padding:5px 0"><div class="ic">${CAT_ICON[(o.tpl || {}).category] || '🎯'}</div><div class="grow"><div class="t">${esc(tplTitle(o.tpl))}</div><div class="s muted">${(o.tpl || {}).type === 'binary_daily' ? `${o.tpl.target} ${t('days')}` : (o.tpl || {}).type === 'once' ? t('type_once') : `${(o.tpl || {}).target} ${esc((o.tpl || {}).unit || '')}`}</div></div></div>`).join('')}<div class="muted small mt">${t('starts_monday')}</div></div>` : ''}
  ...
  function viewProposals() {
    ...
    const pRow = (p, mineFlag) => {
      ...
      return `<div class="item" style="${p.vetoed ? 'opacity:.5' : ''}"><div class="ic">${CAT_ICON[(p.tpl || {}).category] || '🎯'}</div><div class="grow" data-ccinfo="p:${p.id}"><div class="t">${esc(tplTitle(p.tpl))} <span class="muted">›</span></div><div class="s">${esc(authors)} · ${p.votes.length} ${t('votes')}${p.votes.length ? ` (${p.votes.map((v) => esc(member(v).name.split(' ')[0])).join(', ')})` : ''}${p.tpl && p.tpl.source === 'custom' ? ` · <span class="pill">${t('custom').toLowerCase()}</span>` : ''}</div></div><div>${ctr}</div></div>`;
    };
