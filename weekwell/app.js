...
(function () {
  ...
  function viewChallenges() {
    ...
    const pRow = (p, mineFlag) => {
      ...
      return `<div class="item" style="${p.vetoed ? 'opacity:.5' : ''}"><div class="ic">${CAT_ICON[(p.tpl || {}).category] || '🎯'}</div><div class="grow" data-ccinfo="p:${p.id}"><div class="t">${esc(tplTitle(p.tpl))} <span class="muted">›</span></div><div class="s">${esc(authors)} · ${p.votes.length} ${t('votes')}${p.tpl && p.tpl.source === 'custom' ? ` · <span class="pill">${t('custom').toLowerCase()}</span>` : ''}</div></div><div>${ctr}</div></div>`;
    };
  ...
  function viewGroup() {
    ...
    return `
      <div class="card"><div class="row"><div>${ring(groupPct(), t('group_pct'))}</div><div class="grow"><div class="row" style="font-weight:800">${gav(30)} <span>${esc(S.group.name)}</span> ${S.group.admin === S.me ? `<button class="mini ghost" data-gsettings="1">✏️</button>` : ''}</div><div class="row mt" style="align-items:flex-end;height:60px">${ghist || `<span class="muted small">${t('no_items')}</span>`}</div></div></div><div class="row mt"><button class="primary" data-invite="1">➕ ${t('invite')}</button><button class="mini ghost" data-join="1">${t('join_code')}</button></div></div>
  ...
  function memberSheet(id) {
    ...
    sheet(`<div class="row" style="gap:14px">${av(m, 72)}<div class="grow"><div class="pstats"><div><b>${nph}</b><span>${t('photos')}</span></div><div><b>${badgesOf(m.id)}</b><span>🏅</span></div><div><b>${wk}</b><span>${t('weeks_done')}</span></div></div><div class="h2" style="margin:6px 0 0">${esc(m.name)}</div><div class="muted small">${esc(m.location || '')}${m.id === S.group.admin ? ` · ${t('admin')}` : ''}${S.group.paused.includes(m.id) ? ` · ${t('paused')}` : ''}</div><div class="small mt">${esc(m.bio || '')}</div></div></div>
      ...
      ${story}
      <div class="mt"><div class="muted small">${t('goals')}</div>${(m.goals || []).length ? (m.goals || []).map((g) => { const e = g.entries || []; const first = e.length ? e[0].value : null, last = e.length ? e[e.length - 1].value : null; const u = mDef(g).unit; const tr = e.length >= 2 ? trend(g) : 'stable'; const delta = first != null && last != null ? (last - first) : null; const prog = first != null && last != null && g.target !== first ? Math.max(0, Math.min(1, (last - first) / (g.target - first))) : 0; return `<div style="padding:6px 0;border-top:1px solid var(--line)"><div class="row between small"><b>${esc(mName(g))}</b><span class="pill ${tr === 'improving' ? 'acc' : tr === 'attention' ? 'bad' : ''}">${t('trend_' + tr)}</span></div><div class="row between small muted"><span>${delta != null ? (delta > 0 ? '+' : '') + (Math.round(delta * 10) / 10) + ' ' + esc(u) + ' ' + t('since_start') : t('no_items')}</span><span>${t('goal')}: ${g.target} ${esc(u)}</span></div><div class="bar"><i style="width:${prog * 100}%"></i></div></div>`; }).join('') : `<div class="muted small">${t('no_shared_goals')}</div>`}</div>
