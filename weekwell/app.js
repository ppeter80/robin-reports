...
(function () {
  ...
  function viewHome() {
    ...
    const weekActs = S.events.filter((e) => e.type === 'activity' && e.a && S.cur.dates.includes(e.a.date)).sort((a, b) => b.ts - a.ts);
    return `
      ...
      <div class="card"><h3>🎯 ${t('weekly_challenges')} <span class="muted" style="text-transform:none;font-weight:400">· ${sel === today() ? t('today') : sel}</span></h3><div class="days">${days}</div>${items || `<div class="muted">${t('no_items')}</div>`}<div class="muted mt">${t('checkin_hint')}</div>
        <div class="row mt"><button class="mini" data-actadd="1">➕ ${t('act_add')}</button><button class="mini ghost" data-acttoggle="1">⚡ ${weekActs.length} ${WW_STATE.showActs ? '⌃' : '›'}</button></div>
        ${WW_STATE.showActs ? `<div class="mt small"><div class="muted">${t('act_list')}</div>${weekActs.map((e) => `<div class="row" style="padding:5px 0;border-top:1px solid var(--line)">${av(memberFull(e.user), 24)}<span class="grow">${esc((e.a || {}).title || '')}${(e.a || {}).note ? ` <span class="muted">– ${esc(e.a.note)}</span>` : ''}</span><span class="muted">${((e.a || {}).date || '').slice(5)}</span></div>`).join('') || `<div class="muted">${t('act_none')}</div>`}</div>` : ''}</div>
  ...
  function viewGroup() {
    ...
    const evTxt = (e) => e.type === 'rule_proposed' ? t('ev_rule_proposed', { name: member(e.user).name, t: e.title || '' }) : e.type === 'rule_accepted' ? t('ev_rule_accepted', { t: e.title || '' }) : e.type === 'rule_rejected' ? t('ev_rule_rejected', { t: e.title || '' }) : e.type === 'badges' ? t('ev_badges', { name: member(e.user).name, n: e.n || 0, s: e.n || 0 }) : e.type === 'caught_up' ? t('ev_caught_up', { name: member(e.user).name, n: e.n || 0 }) : e.type === 'activity' ? t('ev_activity', { name: member(e.user).name }) + ': ' + ((e.a || {}).title || '') + ((e.a || {}).note ? ' – ' + e.a.note : '') : e.type === 'done' ? t('ev_done', { name: member(e.user).name, title: e.title || '' }) : e.type === 'photo' ? t('ev_photo', { name: member(e.user).name }) + (e.title ? ' – ' + e.title : '') : e.type === 'story' ? t('ev_story', { name: member(e.user).name }) + (e.title ? ' – ' + e.title : '') : e.type === 'pause' ? t('ev_pause', { name: member(e.user).name }) : e.type === 'selected' ? t('ev_selected') : e.type === 'extra' ? t('ev_extra', { name: member(e.user).name }) : e.type === 'join' ? t('ev_join', { name: member(e.user).name }) : e.type;
    const evs = [...S.events].sort((a, b) => b.ts - a.ts).map((e) => `<div class="ev"><div class="row"><div class="ic" style="font-size:20px">${e.type === 'badges' || e.type === 'caught_up' ? badgeSvg(e.n || 0, 30) : e.user ? av(memberFull(e.user), 30) : '🎲'}</div><div class="grow"><div>${esc(evTxt(e))} ${e.proof ? `<span class="pill acc">${t('src_proof')}</span>` : ''}</div><div class="muted small">${new Date(e.ts).toLocaleString(WW_STATE.lang === 'en' ? 'en-GB' : 'sk-SK', { weekday: 'short', hour: '2-digit', minute: '2-digit' })}</div></div></div>${e.photo ? `<img class="evphoto" src="${esc(e.photo)}" alt="" data-view="${esc(e.photo)}">` : ''}
      <div class="k">${C.kudos.map((k) => `<button class="${(e.kudos[k] || []).includes(S.me) ? 'on' : ''}" data-kudos="${e.id}" data-k="${k}">${k}${(e.kudos[k] || []).length ? ' ' + e.kudos[k].length : ''}</button>`).join('')}${e.type === 'activity' && e.user === S.me ? `<button class="ghost" style="margin-left:auto" data-delact="${e.id}" title="${t('delete')}">🗑</button>` : e.photo && e.user === S.me ? `<button class="ghost" style="margin-left:auto" data-delphotoev="${e.id}" title="${t('delete')}">🗑</button>` : ''}</div>
  ...
  // ---------- CHAT ----------
  const CHAT_QUICK = ['👍', '❤️', '😂', '🔥', '💪', '🎉', '👏', '😮'];   /* rýchle reakcie na správu (ako v TdC appke) */
  function viewChat() {
    ...
    const when = (m) => new Date(m.ts).toLocaleString(WW_STATE.lang === 'en' ? 'en-GB' : 'sk-SK', { weekday: 'short', hour: '2-digit', minute: '2-digit' });
    const nameOf = (m) => isRobin(m) ? '🤖 Robin' : member(m.user).name; const preview = (m) => (isRobin(m) ? m.text.replace(/^(📣|🤖)\s*Robin(\[\w\w\])?:?\s*/, '') : m.text).replace(/\s+/g, ' ').slice(0, 80);
    const quote = (m) => { if (!m.replyTo) return ''; const src = S.messages.find((x) => x.id === m.replyTo); return src ? `<div class="qreply">↩︎ <b>${esc(nameOf(src))}</b>: ${esc(preview(src))}</div>` : `<div class="qreply">↩︎ <span class="muted">${t('chat_older')}</span></div>`; };
    const pills = (m) => { const r = m.reactions || {}; const ks = Object.keys(r).filter((k) => r[k].length); return ks.length ? `<div class="rx">${ks.map((k) => `<span class="rxpill ${r[k].includes(S.me) ? 'mine' : ''}" data-mrx="${k}" data-mid="${m.id}" title="${esc(r[k].map((u) => member(u).name).join(', '))}">${k} ${r[k].length}</span>`).join('')}</div>` : ''; };
    const actions = (m, mine) => WW_STATE.openMsg === m.id ? `<div class="msgact ${mine ? 'mine' : ''}"><div class="emorow">${CHAT_QUICK.map((k) => `<span class="ra" data-mrx="${k}" data-mid="${m.id}">${k}</span>`).join('')}</div><div class="btnrow"><span class="rb" data-mreply="${m.id}">${t('chat_reply')}</span>${mine ? `<span class="rb del" data-mdel="${m.id}">${t('chat_del')}</span>` : ''}</div></div>` : '';
    const msgs = S.messages.filter((m) => !robinLang(m) || robinLang(m) === myLang).map((m) => { if (isRobin(m)) return `<div class="msg robin" data-mopen="${m.id}"><div class="who">🤖 Robin</div>${quote(m)}${esc(m.text.replace(/^(📣|🤖)\s*Robin(\[\w\w\])?:?\s*/, ''))}<div class="when">${when(m)}</div>${pills(m)}</div>${actions(m, false)}`; const mine = m.user === S.me; return `<div class="msg ${mine ? 'me' : ''}" data-mopen="${m.id}"><div class="who" style="color:${mine ? 'rgba(255,255,255,.85)' : colorOf(m.user)}">${esc(member(m.user).name)}</div>${quote(m)}${esc(m.text)}<div class="when">${when(m)}</div>${pills(m)}</div>${actions(m, mine)}`; }).join('');
    const rt = WW_STATE.replyTo ? S.messages.find((x) => x.id === WW_STATE.replyTo) : null; const replybar = rt ? `<div class="replybar"><div>${t('chat_replying')} <b>${esc(nameOf(rt))}</b>: ${esc(preview(rt).slice(0, 60))}</div><span data-mreplycancel="1">✕</span></div>` : '';
    setTimeout(() => { const el = $('#chatend'); if (el) el.scrollIntoView(); }, 0);
    return `<div class="card"><h3>${t('chat')} · ${esc(S.group.emoji)} ${esc(S.group.name)}</h3><div class="chat">${msgs || `<div class="muted small">${t('chat_empty')}</div>`}<div id="chatend"></div></div></div><div class="chatin">${replybar}<div class="row" style="align-items:flex-end"><textarea data-chatin placeholder="${t('chat_ph')}" maxlength="500" rows="1" autocomplete="off"></textarea><button class="primary" data-chatsend="1">${t('send')}</button></div></div>`;
  }
  // ---------- PROFIL ----------
  ...
  function gallery(uid, mine) {
    const ph = S.photos.filter((p) => p.user === uid && (p.kind === 'photo' || p.kind === 'activity'));
  ...
  function viewProfile() {
    ...
    return `
      <div class="card">
        <div class="row" style="gap:16px;align-items:center"><div class="avwrap">${avBig(m, 92)}<label title="${t('change_photo')}"><input type="file" accept="image/*" data-avatar hidden><span class="avedit">📷</span></label></div>
  ...
  // #52 aktivity navyše: typ → emoji + predvolená jednotka; emoji sa používa aj na zhodu s bežiacou výzvou (iconFor)
  const ACT_TYPES = [['run', '🏃', 'km'], ['bike', '🚴', 'km'], ['walk', '🚶', 'km'], ['swim', '🏊', 'km'], ['strength', '🏋️', 'min'], ['yoga', '🧘', 'min'], ['team', '⚽', 'min'], ['other', '⭐', 'min']];
  function activitySheet() {
    const todayI = S.cur.dates.indexOf(today()); let date = selDate(); if (S.cur.dates.indexOf(date) > todayI || !S.cur.dates.includes(date)) date = today();
    const sh = sheet(`<div class="h2">⚡ ${t('act_title')}</div><div class="muted small">${t('act_hint')}</div>
      <label>${t('act_type')}</label><select data-a="type">${ACT_TYPES.map(([k, em]) => `<option value="${k}">${em} ${t('act_' + k)}</option>`).join('')}</select>
      <div class="grid2"><div><label>${t('act_value')}</label><input data-a="value" type="number" inputmode="decimal" min="0" step="0.1" placeholder="0"></div><div><label>&nbsp;</label><select data-a="unit"><option value="km">km</option><option value="min">min</option><option value="×">×</option></select></div></div>
      <input data-a="note" maxlength="120" placeholder="${t('act_note_ph')}" class="mt">
      <div class="row mt"><button class="mini" data-a="photo">📷 ${t('act_photo')}</button><span class="muted small" data-a="pname"></span><input type="file" accept="image/*" data-a="file" hidden></div>
      <div class="small mt" data-a="into" style="color:var(--acc)"></div>
      <div class="row mt"><button class="primary" data-a="ok">${t('save')}</button><button data-a="x">${t('cancel')}</button></div>`);
    const g = (k) => sh.querySelector(`[data-a="${k}"]`); let file = null;
    const match = () => { const [, em] = ACT_TYPES.find(([k]) => k === g('type').value) || []; const u = g('unit').value; return S.cur.challenges.find((c) => c.tpl && iconFor(c.tpl) === em && ((c.tpl.type === 'binary_daily' || c.tpl.type === 'once') || String(c.tpl.unit || '').toLowerCase() === u)) || null; };
    const sync = () => { const cc = match(); g('into').textContent = cc ? '→ ' + t('act_into', { t: tplTitle(cc.tpl) }) : ''; };
    g('type').addEventListener('change', () => { const [, , u] = ACT_TYPES.find(([k]) => k === g('type').value); g('unit').value = u; sync(); }); g('unit').addEventListener('change', sync); sync();
    sh.addEventListener('click', async (ev) => {
      const b = ev.target.closest('[data-a]'); if (!b) return; const k = b.dataset.a;
      if (k === 'photo') { const inp = g('file'); inp.onchange = () => { file = inp.files[0] || null; g('pname').textContent = file ? '✔ ' + file.name.slice(0, 24) : ''; }; inp.click(); return; }
      if (k === 'x') { sh.remove(); return; }
      if (k !== 'ok') return;
      const val = +g('value').value; if (!(val > 0)) { g('value').focus(); return; }
      const [ak, em] = ACT_TYPES.find(([x]) => x === g('type').value); const unit = g('unit').value; const cc = match(); b.disabled = true;
      const title = `${em} ${t('act_' + ak)} ${Math.round(val * 100) / 100} ${unit}`;
      await act(async () => { const blob = file ? await shrink(file) : null; await ST.addActivity({ atype: ak, value: val, unit, note: g('note').value.trim(), title, date, blob, cc: cc ? cc.id : null }); sh.remove(); toast('⚡'); });
    });
  }
  function groupSheet() {
  ...
  function memberSheet(id) {
    ...
    sheet(`<div class="row" style="gap:14px">${avBig(m, 72)}<div class="grow"><div class="pstats"><div><b>${nph}</b><span>${t('photos')}</span></div><div><b>${badgesOf(m.id)}</b><span>🏅</span></div><div><b>${wk}</b><span>${t('weeks_done')}</span></div></div><div class="h2" style="margin:6px 0 0">${esc(m.name)}</div><div class="muted small">${esc(m.location || '')}${m.id === S.group.admin ? ` · ${t('admin')}` : ''}${S.group.paused.includes(m.id) ? ` · ${t('paused')}` : ''}</div><div class="small mt">${esc(m.bio || '')}</div></div></div>
  ...
  document.addEventListener('click', (e) => {
    if (!S) return;
    const el = e.target.closest('[data-tab],[data-d],[data-chk],[data-proof],[data-propose],[data-vote],[data-veto],[data-delp],[data-sim],[data-lb],[data-kudos],[data-csend],[data-invite],[data-join],[data-pause],[data-addgoal],[data-lang],[data-export],[data-delacc],[data-reset],[data-signout],[data-view],[data-member],[data-storybtn],[data-photobtn],[data-delphoto],[data-avatar-btn],[data-editgoal],[data-delgoal],[data-catchup],[data-theme],[data-rpropose],[data-rvote],[data-rrevoke],[data-chatsend],[data-mrx],[data-mreply],[data-mreplycancel],[data-mdel],[data-mopen],[data-gsettings],[data-editp],[data-pushon],[data-ccinfo],[data-delphotoev],[data-selectnow],[data-reopen],[data-chtab],[data-again],[data-actadd],[data-acttoggle],[data-delact]');
    if (!el) return; const d = el.dataset;
    if (d.gsettings) { groupSheet(); return; }
    if (d.actadd) { activitySheet(); return; }
    if (d.acttoggle) { WW_STATE.showActs = !WW_STATE.showActs; render(); return; }
    if (d.delact) { if (confirm(t('delete') + '?')) act(() => ST.deleteActivity(d.delact)); return; }
    ...
    if (d.chatsend) { const inp = $('[data-chatin]'); const txt = (inp.value || '').trim(); if (!txt) return; if (PENDING.has('chat')) return; inp.value = ''; inp.style.height = ''; const text = txt.slice(0, 500); const replyTo = WW_STATE.replyTo || null; WW_STATE.replyTo = null; WW_STATE.openMsg = null; if (!C.stub) { S.messages.push({ id: 'tmp' + Date.now(), user: S.me, text, ts: Date.now(), replyTo, reactions: {} }); render(); window.scrollTo(0, document.body.scrollHeight); } return act(() => ST.sendMessage(text, replyTo), 'chat'); }
    if (d.mrx) { WW_STATE.openMsg = null; return act(async () => { await ST.toggleMsgReaction(d.mid, d.mrx); render(); }, 'mrx' + d.mid); }
    if (d.mreply) { WW_STATE.replyTo = d.mreply; WW_STATE.openMsg = null; render(); const inp = $('[data-chatin]'); if (inp) inp.focus(); return; }
    if (d.mreplycancel) { WW_STATE.replyTo = null; render(); return; }
    if (d.mdel) { if (!confirm(t('chat_del_q'))) return; WW_STATE.openMsg = null; return act(async () => { await ST.deleteMessage(d.mdel); render(); }, 'mdel'); }
    if (d.mopen) { WW_STATE.openMsg = WW_STATE.openMsg === d.mopen ? null : d.mopen; render(); const el2 = document.querySelector(`[data-mopen="${d.mopen}"]`); if (el2) el2.scrollIntoView({ block: 'nearest' }); return; }
    ...
    if (d.proof) {
      const sh = sheet(`<div class="h2">${t('proof_add')}</div><label>${t('proof_photo')}</label><div class="row"><button class="mini" data-pf="pick">🖼 ${t('proof_pick')}</button><span class="muted small" data-pf-name></span></div><input type="file" accept="image/*" data-pf-file hidden><label class="mt">${t('proof_strava')}</label><input data-pf-url placeholder="https://www.strava.com/activities/…"><div class="row mt"><button class="primary" data-pf="save">${t('save')}</button><button data-pf="x">${t('cancel')}</button></div>`);
      sh.addEventListener('click', async (ev) => { const b = ev.target.closest('[data-pf]'); if (!b) return; if (b.dataset.pf === 'x') { sh.remove(); return; }
        if (b.dataset.pf === 'pick') { const inp = sh.querySelector('[data-pf-file]'); inp.onchange = () => { const f0 = inp.files[0]; sh.querySelector('[data-pf-name]').textContent = f0 ? '✔ ' + f0.name.slice(0, 24) : ''; }; inp.click(); return; }
  ...
  const TOUCH = 'ontouchstart' in window;   /* mobil: Enter = nový riadok (ako WhatsApp), odoslanie tlačidlom; desktop: Enter = odoslať, Shift+Enter = riadok */
  document.addEventListener('input', (e) => { if (e.target.matches('[data-chatin]')) { e.target.style.height = ''; e.target.style.height = Math.min(e.target.scrollHeight, 140) + 'px'; } });   /* chat: textarea rastie s textom ako WhatsApp, max ~5 riadkov */
  document.addEventListener('keydown', (e) => { if (e.key === 'Enter' && !e.shiftKey && !TOUCH && e.target.matches('[data-chatin]')) { e.preventDefault(); const b = $('[data-chatsend]'); if (b) b.click(); } if (e.key === 'Enter' && e.target.matches('[data-name],[data-loc],[data-ruletext]')) { e.preventDefault(); e.target.blur(); } });
