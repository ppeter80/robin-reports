...
(function () {
  ...
  function viewHome() {
    ...
    const items = S.cur.challenges.map((cc) => {
      ...
      return `<div class="item"><div class="ic">${CAT_ICON[tpl.category] || '🎯'}</div><div class="grow"><div class="t">${esc(tplTitle(tpl))}</div>${tplDesc(tpl)}<div class="s">${cc.catchup ? `<span class="pill warn">${t('catchup_item')}</span> ` : ''}${tplLine(tpl)} ${src} ${tpl.proof === 'required' ? `<span class="pill warn">${t('proof_required')}</span>` : ''}</div><div class="bar"><i style="width:${pct * 100}%"></i></div></div><button class="mini ghost" data-proof="${cc.id}">${t('proof_add')}</button>${ctrl}</div>`;
    }).join('');
  ...
  function viewChallenges() {
    const ph = phase();
    const thisWeek = S.cur.challenges.map((cc) => { const dots = S.cur.dates.map((d) => { const l = S.logs[S.me + '|' + cc.id + '|' + d]; return `<span style="display:inline-block;width:10px;height:10px;border-radius:5px;margin-right:3px;background:${l && (l.done || l.value) ? '#34d399' : '#26282d'}"></span>`; }).join(''); return `<div class="item"><div class="ic">${CAT_ICON[(cc.tpl || {}).category] || '🎯'}</div><div class="grow"><div class="t">${esc(tplTitle(cc.tpl))}</div>${tplDesc(cc.tpl)}<div class="s">${tplLine(cc.tpl)} · ${cc.catchup ? `<span class="pill warn">${t('catchup_item')}</span>` : `<span class="pill acc">${t(SRC_T[cc.source] || 'library')}${cc.votes ? ' ' + cc.votes : ''}</span>`}</div><div class="mt">${dots}</div></div><div class="pct" style="font-weight:800">${P(challengePct(S.me, cc))}</div></div>`; }).join('') || `<div class="muted small">${t('no_items')}</div>`;
