...
(function () {
  ...
  function viewChat() {
    ...
    return `<div class="card"><h3>${t('chat')} · ${esc(S.group.emoji)} ${esc(S.group.name)}</h3><div class="chat">${msgs || `<div class="muted small">${t('chat_empty')}</div>`}<div id="chatend"></div></div></div><div class="chatin row"><textarea data-chatin placeholder="${t('chat_ph')}" maxlength="500" rows="1" autocomplete="off"></textarea><button class="primary" data-chatsend="1">${t('send')}</button></div>`;
  }
  // ---------- PROFIL ----------
  ...
  document.addEventListener('click', (e) => {
    ...
    if (d.chatsend) { const inp = $('[data-chatin]'); const txt = (inp.value || '').trim(); if (!txt) return; if (PENDING.has('chat')) return; inp.value = ''; inp.style.height = ''; const text = txt.slice(0, 500); if (!C.stub) { S.messages.push({ id: 'tmp' + Date.now(), user: S.me, text, ts: Date.now() }); render(); window.scrollTo(0, document.body.scrollHeight); } return act(() => ST.sendMessage(text), 'chat'); }
